/**
 * @fileoverview Brancalonia Covo Macros
 * 
 * Macro predefinite per gestione rapida del sistema Covo.
 * Fornisce 9 macro user-friendly per facilitare l'uso del sistema.
 * 
 * @version 2.0.0
 ...
 * 
 * Fornisce 9 macro per facilitare la gestione del sistema Covo:
 * - Creazione Covo
 * - Status Covo
 * - Gestione Tesoro
 * - Costruzione Granlussi
 * - Raccolta Intrugli
 * - Noleggio Cavalcature
 * - Riparazione Equipaggiamento
 * - Migrazione Covi Vecchi
 * - Aiuto Sistema Covo
 * 
 * @class CovoMacros
 */
...
Hooks.once('ready', async () => {
  try {
    // Verifica che il setting esista prima di usarlo
    const settingExists = game.settings.settings.has('brancalonia-bigat.covoSystemEnabled');

    if (settingExists && game.user.isGM && game.settings.get('brancalonia-bigat', 'covoSystemEnabled')) {
      // Chiedi se creare le macro - usa foundry.applications.api.DialogV2
      const shouldCreate = await foundry.applications.api.DialogV2.confirm({
        window: { title: 'Macro Sistema Covo' },
        content: `
...
window.CovoMacros = CovoMacros;
window['covo-macros'] = CovoMacros; // For module loader