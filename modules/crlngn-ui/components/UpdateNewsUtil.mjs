...
export class UpdateNewsUtil {
  ...
  static getUpdateNewsUrl() {
    const moduleVersion = game.modules.get('brancalonia-bigat')?.version;
    const baseUrl = moduleVersion 
      ? `https://github.com/crlngn/brancalonia-bigat/releases/download/v${moduleVersion}/module-updates.json` 
      : ``;
      // `https://github.com/crlngn/brancalonia-bigat/releases/latest/download/module-updates.json`;
  ...
  static getIdFromRawJson = async () => {
    // Il file module-updates.json non esiste ancora - skip silenzioso
    // Quando sarà creato, questa funzione lo leggerà automaticamente
    return '';
    
    /* Codice originale commentato - da riattivare quando il file esiste
    try {
      const rawUrl = "https://raw.githubusercontent.com/lordbanana89/brancalonia-bigat-master/main/news/module-updates.json";
