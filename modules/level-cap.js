...
ui.notifications.info(\`Recuperate \${improvedFeatures.length} capacità!\`);
  ...
  /**
   * Inizializza il Level Cap System
   * @static
   * @async
   * @returns {Promise<LevelCapSystem>} Istanza del sistema
  ...
  static async initialize() {
    ...
    try {
      ...
      this.instance._state.settingsRegistered = true;

      // Registra hooks
      this._registerHooks();
      this.instance._state.hooksRegistered = true;
      ...
      this._extendActor();
      this.instance._state.actorExtended = true;
      ...
      this.instance?.statistics.errors.push({
        type: 'initialize',
  ...
  static _registerHooks() {
    ...
    if (isV5Plus) {
      ...
      // Compatibilità per versioni precedenti usando renderApplication
      Hooks.on('renderApplication', (sheet, html) => {
        // Verifica che sia una character sheet
        if (!sheet.actor || sheet.actor.type !== 'character') return;
        if (!sheet.constructor.name.includes('ActorSheet')) return;
...
game.brancalonia.levelCap.showEmeriticenzeManagement(actor);
  ...
  async _applyEmeriticenza(actor, emeriticenzaKey) {
    ...
    try {
      ...
      this.statistics.emeriticenzeApplied++;
      this.statistics.emeriticenzeGranted++;
      ...
      this.statistics.emeriticenzeFailures++;
      this.statistics.errors.push({
        type: 'applyEmeriticenza',
        emeriticenza: emeriticenzaKey,
        actor: actor.name,
        message: error.message,
        timestamp: Date.now()
  ...
  _addEmeriticenzeTab(sheet, html) {
    ...
    // Foundry v13 renderActorSheetV2 passes an array of HTMLElements
  ...
  _updateEmeriticenzaStats(emeriticenzaKey) {
    ...
    // Track in statistics
    this.statistics.emeriticenzeByType[emeriticenzaKey] = (this.statistics.emeriticenzeByType[emeriticenzaKey] || 0) + 1;
    ...
    if (mostPopular) {
      this.statistics.mostPopularEmeriticenza = {
        key: mostPopular[0],
        name: this.emeriticenze[mostPopular[0]]?.name,
        count: mostPopular[1]
      };
  ...
  /**
   * Ottieni lo stato corrente del Level Cap System
   * @static
  ...
  static getStatus() {
    return {
      initialized: this.instance?._state.initialized ?? false,
      settingsRegistered: this.instance?._state.settingsRegistered ?? false,
      hooksRegistered: this.instance?._state.hooksRegistered ?? false,
      macrosCreated: this.instance?._state.macrosCreated ?? false,
      actorExtended: this.instance?._state.actorExtended ?? false,
      maxLevel: this.instance?.MAX_LEVEL ?? 6,
      emeriticenzeCount: Object.keys(this.instance?.emeriticenze ?? {}).length,
      version: this.VERSION
    };
  }
  ...
  static getStatistics() {
    return {
      ...this.instance?.statistics,
      uptime: this.instance?.statistics.initTime ? Date.now() - this.instance.statistics.initTime : 0
    };
  }
  ...
  static resetStatistics() {
    ...
    const initTime = this.instance.statistics.initTime;
    this.instance.statistics = {
      emeriticenzeGranted: 0,
      emeriticenzeByType: {},
      levelCapsBlocked: 0,
      ...
      emeriticenzeApplied: 0,
      emeriticenzeFailures: 0,
      macrosCreated: this.instance.statistics.macrosCreated,
      uiRenderings: 0,
      mostPopularEmeriticenza: null,
      lastEmeriticenzaTime: null,
      initTime,
  ...
  /**
   * Ottieni lista di tutte le emeriticenze disponibili
   * @static
   * @returns {Array} Lista emeriticenze
   * 
   * @example
   * const list = LevelCapSystem.getEmeriticenzeList();
   * list.forEach(em => console.log(em.name));
   */
  static getEmeriticenzeList() {
    if (!this.instance) return [];

    return Object.entries(this.instance.emeriticenze).map(([key, data]) => ({
      key,
      name: data.name,
  ...
  /**
   * Ottieni emeriticenze di un attore
   * @static
   * @param {Actor} actor - Attore da verificare
   * @returns {Object} Emeriticenze dell'attore
   * 
   * @example
   * const actor = game.actors.getName("Eroe");
   * const info = LevelCapSystem.getActorEmeriticenze(actor);
   * console.log(info.available); // 2
   */
  static getActorEmeriticenze(actor) {
    if (!actor) return null;

    return {
      taken: actor.getTakenEmeriticenze?.() || {},
      available: actor.getAvailableEmeriticenzeCount?.() || 0,
      total: actor.getEmeriticenzeCount?.() || 0,
      nextXP: actor.getNextEmeriticenzaXP?.() || 0,
  ...
  /**
   * Verifica se un attore è al level cap
   * @static
   * @param {Actor} actor - Attore da verificare
   * @returns {boolean} True se al level cap
   * 
   * @example
   * const actor = game.actors.getName("Eroe");
   ...
   * }
   */
  static isAtLevelCap(actor) {
    if (!actor || !this.instance) return false;

    const maxLevel = game.settings.get('brancalonia-bigat', 'maxLevel') || this.instance.MAX_LEVEL;
    return actor.system?.details?.level >= maxLevel;
  }

  /**
   * Calcola quante emeriticenze può ottenere un attore in base agli XP
   * @static
   * @param {number} xp - Punti esperienza
   * @returns {number} Numero di emeriticenze disponibili
   * 
   * @example
   * const available = LevelCapSystem.calculateAvailableEmeriticenze(20000);
   * console.log(`Emeriticenze disponibili: ${available}`);
   */
  static calculateAvailableEmeriticenze(xp) {
    if (!this.instance || xp < this.instance.BASE_EMERITICENZA_XP) return 0;

    const overflow = xp - this.instance.BASE_EMERITICENZA_XP;
    return Math.floor(overflow / this.instance.EMERITICENZA_STEP);
  }
  ...
  static showReport() {
    ...
    console.log('%c🎭 LEVEL CAP SYSTEM - REPORT', 'color: #8B4513; font-weight: bold; font-size: 14px');
    console.log('%c═══════════════════════════════════════', 'color: #8B4513; font-weight: bold');
    
    console.log('%c\n📊 STATUS', 'color: #4CAF50; font-weight: bold');
    console.log('Version:', status.version);
    console.log('Initialized:', status.initialized);
    console.log('Max Level:', status.maxLevel);
    console.log('Emeriticenze Types:', status.emeriticenzeCount);
    
    console.log('%c\n📈 STATISTICS', 'color: #2196F3; font-weight: bold');
    console.log('Emeriticenze Granted:', stats.emeriticenzeGranted);
    console.log('Emeriticenze Applied:', stats.emeriticenzeApplied);
    console.log('Failures:', stats.emeriticenzeFailures);
    console.log('Level Caps Blocked:', stats.levelCapsBlocked);
    console.log('Dialogs Shown:', stats.dialogsShown);
    console.log('Macros Created:', stats.macrosCreated);
    console.log('UI Renderings:', stats.uiRenderings);
    
    if (stats.mostPopularEmeriticenza) {
      console.log('%c\n🏆 MOST POPULAR', 'color: #FF9800; font-weight: bold');
      console.log(`${stats.mostPopularEmeriticenza.name} (${stats.mostPopularEmeriticenza.count}x)`);
    }
    ...
    console.log('%c\n⏱️ PERFORMANCE', 'color: #9C27B0; font-weight: bold');
