 ...
 * 
 * @module ReputationInfamiaSystem
 * @version 2.0.0
...
class ReputationInfamiaSystem {
  ...
  static async initialize() {
    try {
      ...
      // Registra settings
      instance._registerSettings(); // Fix: metodo corretto (privato)

      // Estendi Actor con nuovi metodi
      instance._extendActor(); // Fix: metodo corretto (privato)
      ...
      instance._registerChatCommands(); // Fix: metodo corretto (privato)
...
game.brancalonia.reputationInfamia._showReputationDialog(controlled);
  ...
  static getStatus() {
    ...
    return {
      ...
      infamiaLevelsCount: Object.keys(instance?.infamiaLevels ?? {}).length,
      reputationTypesCount: Object.keys(instance?.reputationTypes ?? {}).length,
      titlesCount: Object.keys(instance?.titles ?? {}).length,
      version: this.VERSION
    };
  }
   ...
   * 
   * @example
   * ReputationInfamiaSystem.resetStatistics();
   */
  ...
  static showReport() {
    ...
    console.log('%c🎭 REPUTATION-INFAMIA SYSTEM - REPORT', 'color: #8B4513; font-weight: bold; font-size: 14px');
    console.log('%c═══════════════════════════════════════', 'color: #8B4513; font-weight: bold');
    
    console.log('%c\n📊 STATUS', 'color: #4CAF50; font-weight: bold');
    console.log('Version:', status.version);
    console.log('Initialized:', status.initialized);
    console.log('Infamia Levels:', status.infamiaLevelsCount);
    console.log('Reputation Types:', status.reputationTypesCount);
    console.log('Titles:', status.titlesCount);
    
    console.log('%c\n📈 STATISTICS', 'color: #2196F3; font-weight: bold');
    console.log('Infamia Total:', stats.infamiaTotal);
    console.log('Infamia Increased:', stats.infamiaIncreased);
    console.log('Infamia Decreased:', stats.infamiaDecreased);
    console.log('Reputation Total:', stats.reputationTotal);
    console.log('Reputation Increased:', stats.reputationIncreased);
    console.log('Reputation Decreased:', stats.reputationDecreased);
    console.log('Titles Granted:', stats.titlesGranted);
...
// Non usare Hooks.once('init') qui per evitare inizializzazioni doppie

// Crea macro quando il gioco è pronto
Hooks.once('ready', async () => {
  if (game.brancalonia?.reputationInfamia) {
    await game.brancalonia.reputationInfamia._createMacros();
  }
});

// Esporta per compatibilità e scoperta da parte di BrancaloniaCore
window.ReputationInfamiaSystem = ReputationInfamiaSystem;
window.ReputationInfamiaUnified = ReputationInfamiaSystem; // For module loader
window['reputation-infamia-unified'] = ReputationInfamiaSystem; // For module loader